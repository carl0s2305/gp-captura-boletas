    ...
    namespace gp_captura_boletas
    {
        ...
        private void OnInscripcion(object sender, EventArgs e)
        {
            try
            {
                using var frm = new FormInscripcion();
                frm.ShowDialog(this);
            }
            catch (Exception ex)
            {
                MessageBox.Show("No se pudo abrir el módulo de Inscripción:\n" + ex.Message,
                    "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        private void OnCaptura(object sender, EventArgs e)
        {
            try
            {
                using var frm = new FormCapturaCalificaciones();
                frm.ShowDialog(this);
            }
            catch (Exception ex)
            {
                MessageBox.Show("No se pudo abrir el módulo de Captura de Calificacione:\n" + ex.Message,
                    "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }

        private void OnListaGrupo(object sender, EventArgs e)
        {
            try
            {
                using var frm = new FormListaTrimestral();
                frm.ShowDialog(this);
            }
            catch (Exception ex)
            {
                MessageBox.Show("No se pudo abrir el módulo de Listas Trimestrales:\n" + ex.Message,
                    "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
